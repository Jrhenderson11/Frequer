...
if __name__ == '__main__':
    ...
    def proc_emg(emg, moving, times=[]):
        ...
        if (emg == [0,999,0,999,0,999,0,999,0,999,0,999]):
            print ("DOWN")
            fsmodel.set_center_freq(fsmodel.get_center_freq()-1)
            #freqshow.update_center(-1)
        elif ((True in last) and (emg[5]> 800 and emg[6] > 800)): # and (up==False)):
            print ("UP")
            #fsmodel.set_center_freq(fsmodel.get_center_freq()+1)
        
        elif (max(emg[1:]) == emg[2] and emg[2]>100):
           # freqshow.update_center(1)
            fsmodel.set_center_freq(fsmodel.get_center_freq()+1)
            print("HAND" + str(last))
            #fsmodel.set_center_freq(fsmodel.get_center_freq()+1)
      ...
      #  time.sleep(6.0)
       ...
       # if (not lock2.locked()):
        ...
        while True:
            ...
            #if (time.clock() - lasttime > 0.05):    
            #    m.run()
            #    print ("end: " + str(time.clock() - lasttime))
            #    lasttime = time.clock()
            #    n= n+1
            #    print (n)
            ...
            for event in (pygame.event.get()):
                if (event.type is pygame.MOUSEBUTTONDOWN and (time.time() - lastclick) >= CLICK_DEBOUNCE):
                    lastclick = time.time()
                    fscontroller.multiview().click(pygame.mouse.get_pos())
                if event.type == QUIT or (event.type == KEYDOWN and event.unicode == 'q'):
                        raise KeyboardInterrupt()
            # Update and render the current view.
            
            #fscontroller.both_views()[0].render(screen)
